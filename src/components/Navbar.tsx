    ...
    return (
        <div className={`shadow-md w-full sticky top-0 left-0 ${showNavbar ? "" : "hidden"}`}>
            <div className='md:flex bg-nav items-center justify-between py-4 md:px-10 px-7'>
                ...
                <ul className={`bg-nav md:flex md:items-center md:pb-0 pb-12 absolute md:static md:z-auto z-[-1] left-0 w-full md:w-auto md:pl-0 pl-9 transition-all duration-500 ease-in ${open ? 'top-[90px] opacity-100':'top-[-490px] md:opacity-100 opacity-0'} `}>
                    {linkNav.map((li) => (
